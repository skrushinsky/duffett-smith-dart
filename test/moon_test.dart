...
void main() {
  group('Moon true postion', () {
    ...
    for (var c in cases) {
      List<double> coords = c['coords'];
      truePosition(c['djd'], (lambda, beta, delta, hp, dm) {
        test('longitude for djd ${c['djd']}',
            () => expect(lambda, closeTo(coords[0], DELTA)));
        test('latitude for djd ${c['djd']}',
            () => expect(beta, closeTo(coords[1], DELTA)));
        test('delta for djd ${c['djd']}',
            () => expect(delta, closeTo(c['delta'], DELTA)));
        test('parallax for djd ${c['djd']}',
            () => expect(hp, closeTo(coords[2], DELTA)));
        test('daily motion for djd ${c['djd']}',
            () => expect(dm, closeTo(c['motion'], DELTA)));
      });
    }
  });
