...
void truePosition(
    double djd, Function(double, double, double, double, double) callback) {
  final t = djd / 36525;
  final t2 = t * t;
  ...
  callback(lambda, beta, delta, hp, dm);
}
...
double lunarNode(double djd, {bool trueNode = true}) {
  ...
  var nd;
  if (trueNode) {
    final assemble = (terms) => radians(reduceDeg(polynome(t, terms)));
    final assemble_moon = (k) => assemble(moonOrbit[k]);

    final d = assemble_moon('D');
    final m = assemble_moon('M');
    final f = assemble_moon('F');
